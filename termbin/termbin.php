...
# Check the client use the POST method
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
  http_response_code(405);
  die("ERROR: Invalid method, please use 'POST'!");
}
...
// Check if the user is attaching a file
if (isset($_REQUEST["file"])) {

  $finfo = finfo_open(FILEINFO_MIME_TYPE);
  $mime = finfo_buffer($finfo, $entityBody);
  ...
  $paste = array_merge($paste, [
    "attachment" => $data,
    "attachment_name" => $name
  ]);
// If its not a file its just a dump
} else {
  $paste = array_merge($paste, ["paste" => $entityBody]);
}

// Check if we're running in debug mode
if (isset($_REQUST["debug"])) {
  $debug = true;
}
